...
const Sidebar = ({ isCollapsed, isMobileOpen, onCloseMobile }: Props) => {
  return (
    <>
      {isMobileOpen && (
        <div
          className="fixed inset-0 bg-indigo-950/60 z-40 md:hidden"
          onClick={onCloseMobile}
        />
      )}

      <aside
        className={`fixed md:static inset-y-0 left-0 z-50 bg-indigo-950 text-white flex flex-col
      ...
      >
        <div
          className={`p-6 text-2xl font-bold border-b border-indigo-800 whitespace-nowrap overflow-hidden flex items-center justify-between ${
        ...
        >
          ...
          <button
            onClick={onCloseMobile}
            className="md:hidden text-indigo-300 hover:text-white transition"
          ...
          {enlaces.map((enlace) => {
            ...
            return (
              <Link
                ...
                to={enlace.to}
                onClick={onCloseMobile}
                className={`flex items-center gap-3 p-3 rounded hover:bg-indigo-900 hover:text-amber-400 transition ${
